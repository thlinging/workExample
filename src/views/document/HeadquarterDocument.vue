      ...
      <section class="doc-zone">
        ...
        <div class="zone-table">
          ...
          >
            ...
            <template slot="related" slot-scope="record">
              <a href="javascript:;" @click="openRelated(record)">
                {{ record.relatedDocNo || '请选择' }}
              </a>
            </template>
            <template slot="attach" slot-scope="record">
              <a href="javascript:;" @click="downloadAttach(record)">
            ...
            <template slot="action" slot-scope="record">
              <a-button type="link" size="small" @click="handleUploadProcess(record)">
                上传到办理环节
              </a-button>
            </template>
          </a-table>
...
<script>
import UnitSidebar from './components/UnitSidebar.vue'
import DocumentFilterBar from './components/DocumentFilterBar.vue'
import RelatedDocModal from './components/RelatedDocModal.vue'
import {
  getUnitList,
  getIssuedDocuments,
  getReplyDocuments,
...
export default {
  ...
  components: { UnitSidebar, DocumentFilterBar, RelatedDocModal },
  data () {
    return {
      selectedUnitId: '',
      ...
      issuedList: [],
      issuedLoading: false,
      issuedFilter: {}, // 筛选条给的条件，与 selectedUnitId 合并后发请求
      ...
      issuedColumns: [
        ...
        { title: '文件名称', dataIndex: 'title', width: 300, ellipsis: true },
      ...
      replyColumns: [
        ...
        { title: '状态', dataIndex: 'status', width: 90, scopedSlots: { customRender: 'status' } },
        { title: '发文时间', dataIndex: 'sendTime', width: 120 },
        { title: '来函单位', dataIndex: 'fromUnit', width: 130 },
        { title: '办理期限', dataIndex: 'dateLine', width: 120 },
        { title: '对应函件', key: 'related', dataIndex: '', width: 200, scopedSlots: { customRender: 'related' } },
        { title: '附件', key: 'attach', dataIndex: '', width: 160, scopedSlots: { customRender: 'attach' } },
  ...
  created () {
    // 单位列表的首次加载由 UnitSidebar 在 mounted 里 emit('query') 触发，这里不重复调
    this.loadIssued()
    this.loadReply()
  },
  mounted () {
  ...
  methods: {
    ...
    // 单位 + 筛选条条件合成一次查询
    buildParams (filter) {
      return Object.assign({ unitId: this.selectedUnitId }, filter)
    },
    ...
    openRelated (record) {
      this.relatedTarget = record
      this.relatedVisible = true
    },

    handleRelatedOk (issuedDoc) {
      this.relatedSaving = true
      bindRelatedDoc({ replyId: this.relatedTarget.id, issuedId: issuedDoc.id })
        .then(() => {
          // 先本地回填，避免为一个字段重拉整张表
          this.relatedTarget.relatedDocNo = issuedDoc.docNo
          this.relatedVisible = false
          this.$message.success('已关联 ' + issuedDoc.docNo)
        })
        .finally(() => {
    ...
    handleUploadProcess (record) {
      uploadToProcess(record.id).then(() => {
        this.$message.success('已上传到办理环节')
      })
    },
...
// 全屏工作台：用 fixed 铺满顶部导航（64px）以下的整个视口。
// 这样不受 App.vue 内容区 padding 与页脚的影响，左右两栏才能真正等高拉满，
// 页面本身也不产生纵向滚动 —— 滚动只发生在侧栏列表和两张表格内部。
.doc-page {
  position: fixed;
  top: 64px;
  left: 0;
